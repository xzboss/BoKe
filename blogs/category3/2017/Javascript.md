...
1. 属性
   ...
   8. **type**：表示代码块中的脚本语言类型（MIME类型）
      1. 值为`module`即可在代码中使用 import export
<a name="nF36r"></a>
### <noscript></noscript>
...
<a name="Gsz2r"></a>
#### 4.1 概要
六种见原始数据类型：Undefined，Null，Number，String，Boolean，Symbol（符号）<br />复杂类型：Object
<a name="hHtlX"></a>
#### 4.2 区分类型操作符：typeof

1. typeof null;会返回 "object"（因为 null 被认定为是空对象的引用）
<a name="TtBnq"></a>
...
getRawStr`\n${'-'}\t`
```
![image.png](https://cdn.nlark.com/yuque/0/2023/png/33647907/1682304092784-a5674e16-fb51-4357-afe8-fa893e80f808.png#averageHue=%2322252a&clientId=u33c070c0-abf2-4&from=paste&height=107&id=ufb338e58&originHeight=134&originWidth=276&originalType=binary&ratio=1.25&rotation=0&showTitle=false&size=7150&status=done&style=none&taskId=ude2e4d14-42f4-449f-b2d0-8dce19e50ed&title=&width=220.8)
<a name="FtLzo"></a>
#### 4.6 Symbol （TODO：没看完）
...
<a name="h1tKU"></a>
#### 4.7 Object （后面详细讲）
![image.png](https://cdn.nlark.com/yuque/0/2023/png/33647907/1695117734789-7c37f09d-f1f9-4487-8b81-ba45808c5b76.png#averageHue=%23d5d657&clientId=ua6fc91ac-e968-4&from=paste&height=13&id=u77f10903&originHeight=26&originWidth=846&originalType=binary&ratio=2&rotation=0&showTitle=false&size=25203&status=done&style=none&taskId=u1c9cf140-bb81-4664-af3d-100ef0afc66&title=&width=423)
<a name="O2dIo"></a>
### 操作符
<a name="a2PUp"></a>
####  -- ++ + - === 等
...
-0 + +0 	 // +0
```
![image.png](https://cdn.nlark.com/yuque/0/2023/png/33647907/1694088198889-995629d9-c471-40ea-b5e9-5f0cdc84c8fa.png#averageHue=%23fdfdfc&clientId=u4719d36b-d953-4&from=paste&height=207&id=u2232ec52&originHeight=414&originWidth=832&originalType=binary&ratio=2&rotation=0&showTitle=false&size=27869&status=done&style=none&taskId=ua71d46a9-cb0a-4859-a993-6a11f7a9e9d&title=&width=416)<br />null 与 undefined 不会进行类型转换，但是 null == undefined<br />NaN == NaN 不会调用 Number(NaN)
<a name="VQC5E"></a>
### 语句
<a name="AwyWe"></a>
...
```
> 真不易！
> 现在好像变了很多，众口不一：[https://juejin.cn/post/6844903682283143181#heading-9](https://juejin.cn/post/6844903682283143181#heading-9)
...
<a name="jYr9f"></a>
### Date
![image.png](https://cdn.nlark.com/yuque/0/2023/png/33647907/1694521013145-4da60c29-b530-4b51-a080-da8b2ad97f55.png#averageHue=%23faf7f3&clientId=u03dac2c9-2413-4&from=paste&height=15&id=ue7e4dc35&originHeight=30&originWidth=754&originalType=binary&ratio=2&rotation=0&showTitle=false&size=9554&status=done&style=none&taskId=ub749daa9-bf84-447d-9b55-ab5a0a9709d&title=&width=377)
<a name="t2URA"></a>
### 原始值包装类型
```javascript
const s = 'xuzhou'
...
<a name="ITlh6"></a>
#### ![image.png](https://cdn.nlark.com/yuque/0/2023/png/33647907/1695118122522-6ce64172-8575-439d-b54e-5045e75d0234.png#averageHue=%23dad5cf&clientId=ua6fc91ac-e968-4&from=paste&height=14&id=u77674eaa&originHeight=28&originWidth=826&originalType=binary&ratio=2&rotation=0&showTitle=false&size=24781&status=done&style=none&taskId=ud525ec4a-e8c6-4045-8aab-00877df3b45&title=&width=413)
<a name="v78DD"></a>
#### from 与 of
```javascript
const obj = {
...
> ![image.png](https://cdn.nlark.com/yuque/0/2023/png/33647907/1696592748157-1773ed25-458c-4db8-a75e-b050603cf9f8.png#averageHue=%23fbf9f7&clientId=u0802f2d1-e9d2-4&from=paste&height=175&id=u0ec690bd&originHeight=350&originWidth=588&originalType=binary&ratio=2&rotation=0&showTitle=false&size=73935&status=done&style=none&taskId=u01dab8e6-7415-4280-98df-de1febe4ea3&title=&width=294)
> 迭代器不与可迭代对象某个快照绑定，意味着迭代时改变对象，迭代结果会收影响，而迭代器内部维护着对可迭代对象的引用，也就会阻止垃圾回收

<a name="qJpOP"></a>
#### 提前关闭迭代
原生的可迭代对象都不能提前关闭，下一次迭代任然继续上一次，及时 return 方法执行并返回 { done: true }
```javascript
const arr = [1, 2, 3, 4, 5]
...
> 访问器属性 get set 不能与数据属性value 或 writable 同时配置
...
> 所以最后 实例 person 与构造函数 Person 并没有直接联系

<a name="X4MBI"></a>
#### 各种关于原型的方法以及遍历键的方法
| Object方法 | <br /> | <br /> |
| --- | --- | --- |
| getPrototypeOf | 获取对象隐式原型 | <br /> |
| setPrototypeOf(obj, proto) | 设置对象的隐式原型 | 严重影响性能 |
| create(proto) | 创建对象并为其指定对象 |  |
| hasOwnProperty | 确定某属性在对象身上还是在原型上 |  |
|  |  |  |
| 获取对象键的方式 |  |  |
| for in | 获取对象及其原型的可枚举属性 | 顺序不确定 |
| Object.keys | 获取对象的可枚举属性 | 顺序不确定 |
